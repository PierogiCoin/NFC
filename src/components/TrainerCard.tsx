...
const TrainerCard = ({ name, specialty, bio, imageUrl }) => {
  // fallback image
  const fallbackImg = 'https://placehold.co/150x150/CCCCCC/000000?text=Trener';
  // use state to handle image error for next/image
  const [imgSrc, setImgSrc] = React.useState(imageUrl || fallbackImg);
  React.useEffect(() => {
    setImgSrc(imageUrl || fallbackImg);
  }, [imageUrl]);

  return (
    <div className="group relative rounded-2xl p-[2px] bg-gradient-to-tr from-blue-500 via-sky-400 to-blue-700 shadow-lg hover:shadow-xl transition-all duration-500 hover:scale-[1.02]">
      <div className="flex flex-col sm:flex-row items-center bg-white dark:bg-neutral-900 p-6 rounded-2xl space-y-4 sm:space-y-0 sm:space-x-6">
        ...
        <div className="text-center sm:text-left max-w-lg">
          ...
          <p className="text-blue-600 dark:text-blue-400 font-semibold text-sm uppercase tracking-wide mb-2">{specialty}</p>
